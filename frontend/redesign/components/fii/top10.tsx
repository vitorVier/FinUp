  ...
  return (
    <>
        ...
        <div className="mb-5 flex w-fit gap-1 rounded border border-border p-1">
          <button
            ...
            className={`rounded px-3 py-1.5 text-xs font-semibold transition-colors ${
              sortMode === "tradicional" ? "bg-secondary text-foreground" : "text-slate-400 hover:text-foreground"
            }`}
          >
            Nota 2 em 1
          </button>
          <button
            ...
            className={`rounded px-3 py-1.5 text-xs font-semibold transition-colors ${
              sortMode === "fuzzy" ? "bg-secondary text-foreground" : "text-slate-400 hover:text-foreground"
            }`}
          >
            Nota fuzzy
          </button>
      ...
      <div className="grid w-full grid-cols-1 gap-3 lg:grid-cols-3">
        {leader && (
          <FundCard fund={leader} onSelect={onSelect} className="lg:col-span-2 lg:row-span-2" large />
        )}
...
function FundCard({
  ...
  return (
    <Card
      ...
      className={`group relative cursor-pointer transition-colors hover:border-primary/50 focus:outline-none focus:ring-1 focus:ring-primary/40 ${className}`}
    >
      <CardContent className={large ? "p-6 pt-6" : "p-4 pt-4"}>
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <span className={`inline-flex h-5 items-center justify-center rounded px-2 text-[9px] font-bold ${rankStyle.badge}`}>
              {rankStyle.label}
            </span>

            <div className="mt-2.5 flex min-w-0 items-center gap-1">
              <h3 className={`truncate font-extrabold tracking-tight text-foreground ${large ? "text-2xl" : "text-base"}`}>
                {fund.Papel}
              </h3>
              <ArrowUpRight
            ...
            <p className="mt-0.5 truncate text-[11px] text-slate-400">{fund.Segmento}</p>
          </div>

          <div className="shrink-0 text-right">
            <p className="text-[8px] font-semibold uppercase tracking-[0.1em] text-slate-400">Nota</p>
            <p className={`mt-0.5 font-extrabold leading-none tabular-nums ${rankStyle.score} ${large ? "text-3xl" : "text-xl"}`}>
              {fund.Nota_Final.toFixed(1)}
            </p>
            <p className="mt-0.5 text-[9px] text-slate-400">fuzzy</p>
          </div>
        </div>

        <div className="mt-4 flex min-h-6 items-center justify-between gap-2">
          <RecommendationBadge value={fund.Recomendacao} />
          <span className="hidden text-[9px] text-slate-400 transition-opacity group-hover:block">Ver detalhes</span>
        </div>

        <div className={`mt-4 grid gap-2 ${large ? "grid-cols-4" : "grid-cols-2"}`}>
          ...
          <Metric label="Alvo mínimo" value={formatBRL(fund.Preco_Alvo_Min)} icon={Target} />
        </div>

        <div className="mt-4 flex items-center justify-between gap-2 border-t border-border pt-3">
          <span className="truncate text-[9px] text-slate-400">Clique para analisar</span>
          <ArrowUpRight
